...
import { Constants, getRandom, heightRatio, width } from '../utils'

type UpdateObstaclePros = {
  time: any
  dispatch: any
}

const UpdateObstacle = (entities: any, { time, dispatch }: UpdateObstaclePros) => {
  ...
  for (let i = 1; i <= 2; i++) {
    if(
      ...
      Matter.Body.translate(entities['Obstacle' + i].body, {
        x: -4,
        y: 0
      })
    }
